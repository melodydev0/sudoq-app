...
class AuthService {
  ...
  static Future<UserCredential?> signInWithApple() async {
    try {
      ...
      // Post-sign-in steps are non-fatal: if Firestore rules, network, or sync
      // fails, the user is still authenticated via Firebase Auth. Returning
      // null here would make the caller think sign-in failed and show
      // "sign in before purchase" even though the user is signed in.
      try {
        await _createOrUpdateUserProfile(userCredential.user!);
      } catch (e) {
        debugPrint('Apple post-sign-in profile sync failed (non-fatal): $e');
      }
      ...
      try {
        ...
        debugPrint('Apple post-sign-in cloud sync failed (non-fatal): $e');
      }

      try {
        await EntitlementService.refreshFromCloud();
      } catch (e) {
        debugPrint('Apple post-sign-in entitlement refresh failed (non-fatal): $e');
      }
